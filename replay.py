...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def update_weights(self, maps, new_weights):
        for id, weight in zip(samples, new_weights):
            self.errors.set_value(id, self._process_weight(weight))
...
if __name__ == '__main__':
    replay_buffer = PrioritizedReplayBuffer(capacity=3, alpha=1, beta=1)
    replay_buffer.add_sample({'data': 1}, init_weight=0.8)
    replay_buffer.add_sample({'data': 2}, init_weight=0.4)
    replay_buffer.add_sample({'data': 3}, init_weight=0.2)
    for _ in range(10):
        samples = replay_buffer.sample(num_samples = 3)
